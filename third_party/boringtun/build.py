...
def _ensure_win_clang_shim(binpath: Path, shim_dir: Path) -> Path:
    ...
    dst = shim_dir / CLANG
    # BNES: Windows often locks this ~100MB clang copy (Defender / leftover
    # cargo). Unlink then fails with WinError 5 and aborts create_dist.
    # Reuse a same-sized existing shim, or keep it if unlink is denied.
